function (user, context, callback) {
   ...
   * Unknown at 2024-02-01T18:14:17.884Z
   */
  ...
  var config = {
    endpoints: {
      linking: 'https://custom-linking.westondemos.co.uk/',
  ...
  function linkAccounts() {
    ...
    return verifyToken(secondAccountToken, config.token.clientSecret)
      .then(function(decodedToken) {
        ...
        if (user.email !== decodedToken.linkemail) {
          console.error(LOG_TAG, 'User: ', decodedToken.linkemail, 'tried to link to account ', user.email);
         // context.redirect = {
        ...
        var linkUri = config.endpoints.userApi+'/'+decodedToken.linkuserid+'/identities';
        var headers = {
  ...
  function promptUser() {
    return searchUsersWithSameEmail().then(function transformUsers(users) {
      return users.filter(function(u) {
        return u.user_id !== user.user_id && u.identities[0].connection !== "my-test";
      }).map(function(user) {
        return {
          userId: user.user_id,
          email: user.email,
  ...
  function createToken(tokenInfo, targetUsers) {
    ...
    var userSub = {
      ...
      email: user.email,
      base: auth0.baseUrl,
      connection: targetUsers[0].connections[0],
      login_hint: targetUsers[0].email
